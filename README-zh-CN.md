...
- 搜索地点，并通过点击地图查看坐标。
- 将 GCJ-02 坐标转换为 WGS84 和 BD-09。
- 切换标准底图与卫星底图。
- 导入包含点、线或面的 GeoJSON `FeatureCollection` 文件。
- 绘制矩形区域，将当前视图或选中区域导出为 PNG。
...
凭据仅保存在当前标签页的 `sessionStorage` 中，不会进入源代码、构建产物、分析服务或应用日志。按照高德 JavaScript API 的工作方式，浏览器仍会将凭据直接发送给高德服务。
